...
int judge(string s1,string s2){
    int i = 1, j;
    for(string A = s1; A.size(); i++,A=mp[A].father){
        j = 1;
        for(string B = s2; B.size(); j++,B = mp[B].father){
            if(i >= 5 && j >= 5) return 1;
...
int main(){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(lname.back() == 'm') mp[fname].sex = 1;
            ...
            string buf = lname.substr(0,lname.size()-4);
            mp[fname] = {1,buf};
        }else if(lname.back() == 'r'){
            string buf = lname.substr(0,lname.size()-7);
            mp[fname] = {-1,buf};
        }
    }
    scanf("%d",&m);
